...
A process that ensures that the reliability of data transfer on an unreliable network. When sending data, additional "noise" may interfere or corrupt data trasfer. Error detection identifies the errors in transfer while data correction reconstructs the data. This process was first created by Richard Hamming back when code was inputted using punch cards. If you want to see how Hamming was able to do this, check out [this](https://www.techradar.com/news/computing/how-error-detection-and-correction-works-1080736) article. See more [here](https://en.wikipedia.org/wiki/Error_detection_and_correction).
...
Also known as PAN, these networks are meant for a connection between one person's devices. It can include computers, smartphones, tablets, and [[Internet History Support Info#Personal Digital Assistant|personal digital assistants]] (not to be confused with virtual assistants -- Siri, Alexa, Cortana, and Google Assistant). A PAN may be used over a wireless network (==IrDA==, Wireless USB, Bluetooth, ==ZigbBee==) making it a WPAN. 

## Personal Digital Assistant
Commonly called a PDA. It is essentially a handheld PC. I include this here only because I don't know how long this database will be used. Even when I was growing up, PDAs were already being out performed by smartphones. In fact, the only PDA I have ever seen is the one my mom gave me that she no longer needed. A PDA is a digital notepad that uses a large digital display (and usually a few buttons similar to Android phones). Most are able to connect to the internet to allow for connections to the web. Most PDAs also came with a small stylus to help with touch screen functionallity. See more [here](https://en.wikipedia.org/wiki/Personal_digital_assistant).

## Infrared Data Association
Commonly called the IrDA. It refers to a set of protocols used for wireless data transmission over infrared signals. The downside to this technology is that it requires line of sight, can only transmit in a small range. Beacuse of this, this data is often un-encrypted and can easily be intercepted by a hacker. The upside to IrDA is that is has an extremely low ==bit error rate==. IrDA was started by a group of about 50 companies. See more [here](https://en.wikipedia.org/wiki/Infrared_Data_Association).  

## Bit Error Rate
Also called BER. The BER is the number of bits of data added by disrupting noise, interference, distortion, or synchronization errors during transmission. While one bit may not seem like a big disruption, just shifting the code by one bit will likely make it completely unreadable by the computer. Having a low BER means that methods like Hamming Code is not required to fix the errors. 